...
class AdderTest extends \PHPUnit_Framework_TestCase {
  ...
  public function testUserHasMinRole() {
    ...
    $this->assertFalse(
      user_has_min_role($this->user, 'Maintainer'),
      'User should have at least the Maintainer role'
    );
  }

  public function testHomePageCallback() {
    $output = home_page_callback();
    $this->assertEquals(
      gettype($output),
      'array',
      'Output should be of type array'
    );
    $this->assertEquals(
      gettype(render($output)),
      'string',
      'Rendered output should be of type string'
    );
  }
